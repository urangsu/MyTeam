...
enum BuiltInKoreanSkills {
    ...
    private static let characterCountSkill = SkillManifest(
        ...
        name: "한국어 글자 수 세기",
        version: "1.1",
        description: "한국어 텍스트의 공백 포함/제외 글자 수, UTF-8 bytes, 한국 제출폼 기준 bytes를 완전 로컬로 즉시 계산합니다. 외부 API 호출 없음.",
        locale: "ko-KR",
        category: .koreanWriting,
        triggers: ["글자 수", "byte", "바이트", "자소서", "NEIS", "글자수"],
        allowedScopes: [.chatBasic],
        requiredPermissions: [],
        requiredLogin: false,
        riskLevel: .safeReadOnly,
        promptTemplate: "사용자가 한국어 텍스트의 글자 수 계산을 요청했습니다. 이 스킬은 완전 로컬 처리이며 외부 API를 호출하지 않습니다. 공백 포함/제외 글자 수, UTF-8 bytes, 한국 제출폼 기준 bytes, 줄 수, 문단 수를 계산해 주세요.",
        outputType: .chat,
        isBuiltIn: true,
        defaultEnabled: true,
        requiresApprovalEveryRun: false
