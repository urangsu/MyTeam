...
""",
    ...
    private static let accountingTaxSkill = SkillManifest(
        ...
        requiredLogin: false,
        riskLevel: .safeReadOnly,
        promptTemplate: "사용자가 한국 세금·세무 계산 참고 정보를 요청했습니다. 해당 세금의 종류, 세율, 계산 방법, 참고용 계산 결과를 안내하세요. 반드시 '세무·회계 판단은 실제 증빙과 계약 구조에 따라 달라질 수 있으므로 전문가 확인이 필요합니다'라는 면책 조항을 포함하세요.",
        outputType: .chat,
        isBuiltIn: true,
        defaultEnabled: true,
        requiresApprovalEveryRun: false
