...
class ConversationMemory {
    ...
    static func buildPersonalResponsePolicy(
        ...
        case .supportive:
            let comfortExample = CharacterDialogues.lines(for: agentID, event: .userNeedsComfort).first?.text ?? "많이 힘드셨겠어요. 지금은 잠깐 숨을 고르셔도 괜찮아요."
            conversationalGuidance = "\(ConversationReplyPolicy.promptDirective(for: .supportive)) 캐릭터의 따뜻한 결은 '\(comfortExample)'처럼 유지하되 문장을 그대로 복사하지 마세요. 사용자를 평가하거나 참으라고 몰아붙이지 마세요."
