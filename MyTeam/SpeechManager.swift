...
final class SpeechManager: ObservableObject, @unchecked Sendable {
    ...
    private func bubbleSpeechPlaybackSamples(
        ...
        AppLog.info("[BubbleSpeechEffect] mode=sourceAlignedSyllabic strength=\(decision.strength.rawValue) wetMix=\(String(format: "%.2f", decision.wetMix)) reason=\(decision.reason) agentID=\(agentID ?? "nil") sourceSamples=\(sourceSamples.count) renderedSamples=\(samples.count)")
        return samples
    }
