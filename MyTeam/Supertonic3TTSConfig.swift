...
// - 모델 필요: App Store/Direct는 bundled model, Developer는 외부 cache 허용
...
enum Supertonic3TTSConfig {
    ...
    /// Developer-only Supertonic3 ONNX 모델 디렉토리 (Python SDK 기본 경로 참조)
    /// 샌드박스 환경에서 homeDirectoryForCurrentUser는 컨테이너를 반환하므로
    /// ProcessInfo.processInfo.environment["HOME"]로 실제 홈 디렉토리를 사용한다.
    /// App Store / Direct runtime에서는 Supertonic3ModelSourcePolicy를 통해 bundled model을 사용한다.
    ...
    nonisolated static let requiredModelFiles: [String] = [
    ...
    nonisolated static let availableVoicePresets: [String] = [
    ...
    nonisolated static let licenseStatus: String = "MIT sample code + OpenRAIL-M model — product compliance review pending"
    nonisolated static let isLicenseVerifiedForAppStore: Bool = false
    nonisolated static let isModelRedistributionApproved: Bool = false
    nonisolated static let isCommercialProductGateApproved: Bool = false
}
