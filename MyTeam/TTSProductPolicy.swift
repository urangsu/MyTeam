...
enum TTSProductPolicy {
    ...
    static var userFacingTTSEnabled: Bool {
        userFacingTTSEnabled(for: AppReleaseProfile.current)
    }

    static func userFacingTTSEnabled(for profile: AppReleaseProfile) -> Bool {
        switch profile {
        case .appStore:
            return Supertonic3ReleaseGate.isAppStoreApproved && canShipAsProductFeature
        case .debug, .directDownload, .powerUser:
            return true
        }
    }

    static var labOnlyEnabled: Bool {
        labEnabled(for: AppReleaseProfile.current)
    }

    static func labEnabled(for profile: AppReleaseProfile) -> Bool {
        profile.policy.showsDeveloperDiagnostics
    }
