...
#### Round 56A-60H — Character Team Identity + Killer Flow Productization Pack

Round 51A-55H 성장/ASO/프라이버시 문구를 실제 앱 내 UX copy와 기능에 반영한다.

목표:

- Round 51A-55H 성장/ASO/프라이버시 문구를 앱 내 UX copy에 반영
- "내 컴퓨터 안의 AI 업무 팀" 포지셔닝을 첫 화면/설정/브리핑에 반영
- "회의록·체크리스트·보고서"를 첫 핵심 기능으로 정리
- 치코 중심 기본 캐릭터 경험 강화
- 캐릭터를 장식이 아니라 업무 역할 기반 팀원으로 보이게 정리
- 스프라이트 없는 캐릭터/DLC Release 노출 정책 정리
- Starter Action을 회의록 중심 killer flow로 재정렬
- 첫 artifact 생성 후 next action flow 강화
- Settings / Connector Center / Local-only mode copy를 TruthfulPrivacyCopyPolicy 기준으로 통일
- "외부 서버 없음 / 완전 로컬 / 내 기기 안에서만" 과장 표현 제거

진행 체크리스트:

- [ ] 1. 대상 파일 목록 정리 및 현재 상태 확인 (FirstLaunchBannerView, LocalOnlyModeCardView, StarterActionStripView 등)
- [ ] 2. 핵심 문구 치환 (내 컴퓨터 안의 AI 업무 팀 등) — FirstLaunchBannerView, SettingsView, DailyBriefingCardView에 반영
- [ ] 3. 과장 표현 제거 (외부 서버 없음 → MyTeam 자체 서버에 파일을 저장하지 않습니다 등)
- [ ] 4. Killer Flow 구현 — Starter Action 순서 고정: 회의록 양식 → 체크리스트 → 파일 읽기 → 오늘 할 일
- [ ] 5. First Result Activation Flow 강화 — 요약하기, 표로 바꾸기, 체크리스트로 바꾸기, Finder에서 보기 action 추가
- [ ] 6. 치코 중심 기본 캐릭터 정책 — 기본 대표, 문서/할일 정리 역할 명시
- [ ] 7. 캐릭터 DLC 노출 정책 — Release에서 스프라이트 없는 캐릭터는 대표 UI에 미노출
- [ ] 8. SettingsView 간결화 — API key 상태, 로컬 기능, 연결 기능, 위험 작업 차단 정보만 남김
- [ ] 9. Connector Center 상태 모델 정리 — available, readOnly, planned, requiresApproval, blocked, unavailable
- [ ] 10. Google Calendar / Gmail copy 정리 — "준비 중" + "자동 실행하지 않음"
- [ ] 11. RuntimeDiagnostics 보강 — truthfulPrivacyCopyPolicy, killerFlowMeetingMinutesReady, chikoDefaultCharacterEnabled 등 추가
- [ ] 12. RouterBurnInSuite / ToolContractValidator 보강 — 회의록/체크리스트/오늘할일, blocked actions 테스트 케이스 추가
- [ ] 13. DEVLOG 반영 — Round 56A-60H 완료 내용 기록
- [ ] 14. Build 검증 — Debug, Release 모두 성공, warning 0 유지

### Next

#### Round 61A — Character Asset Production + Screenshot Readiness Pack

목표:

- 치코 외 최소 2~3개 캐릭터 스프라이트 제작/반영 준비
- 앱스토어 스크린샷용 화면 구성
- 캐릭터별 역할 차별화 강화
- DLC 판매 가능성 재검토
