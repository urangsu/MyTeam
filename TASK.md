...
#### Round 76A-95Z — Character Asset Pipeline + Internal Product Review + Release Gate Audit Mega Pack

목표:

- ToolExecutor Swift 6 async/actor warning 제거 또는 구조 리스크 없이 문서화
- CharacterAssetManifest / CharacterAssetAvailability / ReleaseVisibleCharacterPolicy 구현
- 치코 중심 기본 캐릭터 경험을 asset-aware 구조로 정리
- placeholder character / DLC / purchase UI Release 노출 차단을 validator로 검수
- First result activation / ArtifactCardView next action 정책 검수
- StarterAction killer flow route 검수
- Connector write / unavailable / planned capability 검수
- StoreKit/paywall surface 검수
- Privacy copy forbidden phrase 검수
- App Store screenshot readiness 검수
- Deployment target / copyright / permission copy 검수
- RuntimeDiagnostics release/debug summary 검수
- Preflight script 작성
- InternalReviewReport 작성
- MarketingReviewFollowup 작성
- PMReviewFollowup 작성
- QA 없이 Debug / Release build warning 0 또는 warning fully documented 상태 유지

진행 체크리스트:

- [ ] 1. TASK.md 대정리 (Round 76A-95Z Now/Next 전환)
- [ ] 2. 시작 상태 확인 (git status, main 동기화)
- [ ] 3. ToolExecutor Swift 6 warning 분석 및 제거/문서화
- [ ] 4. CharacterAssetManifest.swift 신규 구현
- [ ] 5. CharacterAssetAvailability.swift 신규 구현
- [ ] 6. ReleaseVisibleCharacterPolicy.swift 신규 구현
- [ ] 7. CharacterCatalog asset-aware 정책 연결
- [ ] 8. Chiko default experience 강화
- [ ] 9. First result activation 정책 검수 강화
- [ ] 10. Connector / write capability 검수 강화
- [ ] 11. StoreKit / Paywall / DLC surface 검수 강화
- [ ] 12. Privacy copy forbidden phrase 검수
- [ ] 13. Deployment target strategy 재검수
- [ ] 14. Copyright / permission copy 재검수
- [ ] 15. Screenshot readiness 검수 (ScreenshotSurfaceAudit.md)
- [ ] 16. RuntimeDiagnostics 검수 필드 완성
- [ ] 17. Preflight script 작성 (scripts/preflight_round76.sh)
- [ ] 18. InternalReviewReport 작성
- [ ] 19. MarketingReviewFollowup 작성
- [ ] 20. PMReviewFollowup 작성
- [ ] 21. ToolContractValidator 최종 보강
- [ ] 22. RouterBurnInSuite 최종 보강
- [ ] 23. ReleaseWarningAudit 최종화
- [ ] 24. SystemBottleneckAudit 반영
- [ ] 25. DEVLOG 반영
- [ ] 26. 빌드 및 Preflight 실행 + 최종 커밋

### Next

#### Round 96A — Visual Character Asset Production Pack

목표:

- 치코 외 최소 2~3개 캐릭터 스프라이트 실제 제작/반영
- 캐릭터별 idle/working/success 상태 반영
- 앱스토어 스크린샷 후보 화면 캡처 준비
- 캐릭터 DLC 상품 노출 재검토

### Recently Completed

- Round 61A-75H — Product Surface Completion + Character System + Launch Asset Readiness Mega Pack (RuntimeDiagnostics 24 fields, ToolContractValidator DLC/product policy, RouterBurnInSuite killer flow, CharacterAssetSpec/RosterPlan/ScreenshotPlan/ChikoSpec/DLCGatePolicy docs, copyright set in pbxproj, DeploymentTargetStrategy, ReleaseWarningAudit) ✅
- Round 56A-60H — Character Team Identity + Killer Flow Productization Pack (Growth copy integration, StarterAction, FirstLaunchBannerView, LocalOnlyModeCardView, CharacterCatalog Chiko positioning, TruthfulPrivacyCopyPolicy enforcement) ✅
- Round 51A-55H — Growth Positioning and Truthful App Store Copy (6 growth documents, ASO, landing copy, privacy policy) ✅
- Round 43A-47H — Product Completion Without QA Pack (merged to main at daa2b52) ✅
...
### pbxproj File Inclusion Issue (Priority: High)

**상황**: 9개의 Swift 파일이 `/Users/su/Desktop/MyTeam/MyTeam/` 폴더에 존재하지만 Xcode project의 `MyTeam` target에 포함되지 않음.

**누락된 파일**:
1. FirstLaunchState.swift
2. FirstLaunchStateProvider.swift
3. FirstLaunchBannerView.swift
4. LocalOnlyModeCardView.swift
5. RuntimeCapabilityMode.swift
6. StarterAction.swift
7. StarterActionDispatcher.swift
8. StarterActionStripView.swift
9. Color+Hex.swift

**증상**: 
- 파일들이 디스크에 존재하지만 Swift compiler가 인식하지 못함
- `xcodebuild` 빌드 시 타입 미발견 에러 발생 (e.g., "cannot find type 'FirstLaunchState'")
- 이전 Round 43A-47H 완료 후 web Claude Code에서 생성된 파일들인데, pbxproj merge 과정에서 누락됨

**원인 분석**:
- Round 43A-47H 통합 시 `git checkout --theirs` 선택하여 remote 버전 수용
- remote 버전의 pbxproj에는 이 9개 파일의 명시적 참조가 없었음 
- local 환경에서 `xcodebuild`는 Build Settings에 명시된 파일만 컴파일함

**해결 방법** (2가지):
1. **Xcode UI (권장)**: Xcode에서 `File > Add Files to "MyTeam"...` 메뉴로 9개 파일을 드래그앤드롭하여 추가
2. **명령행**: xcodeproj gem 또는 pbxproj 텍스트 편집으로 파일 참조 추가 (인코딩 문제 주의)

**다음 단계**:
- Step 3 이후 모든 작업이 이 이슈에 블록됨
- pbxproj 해결 후 FirstLaunchStateProvider 등의 타입이 정상 인식됨
