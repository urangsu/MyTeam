...
목표:
- workflow 완료 메시지와 ChatLog.artifactIDs 연결 (UniversalDocument, PrivacyTerms, AppLaunch)
- WorkResultCardView inline artifact 표시 (관련 artifact를 카드 내부에 포함)
- ArtifactCardView compact inline mode (한 줄 컴팩트 레이아웃)
- AgentChatView 하단 artifact 목록 중복 제거
- SkillResultRendererView generic card fallback (5줄+, 마크다운 헤더/표/체크리스트)
- 긴 결과 접기/펼치기 UX polish
- RuntimeDiagnostics 4개 신규 필드 (inline artifacts / artifact linking / skill fallback / dedup)
- ToolContractValidator 3개 신규 validator
- docs: ResultPresentationPolicy "Inline Artifact Linking" 섹션
