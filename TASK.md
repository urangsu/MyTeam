...
- App Store packaging 문서 보강 (AppStorePackagingChecklist.md 완성)
- Release diagnostics minimization 재점검
- first launch / no-key / offline UX 점검
- sandbox / file access final guard 정리
- external write / destructive action final guard 정리
- StoreKit / entitlement 상태 문서화
- startup / termination crash-prone path 점검
- Release warning audit 마감
- Submission runtime QA checklist 정리
- 빌드 및 커밋

### Next

#### Round 41A — First Launch + Onboarding Polish Pack

목표:

- 첫 실행 온보딩 UX polish
- API key / local-only mode 안내
- 팀 생성 / 캐릭터 선택 / 파일 작업 시작 흐름 단순화
- 사용자가 1분 안에 첫 결과물을 만들 수 있게 정리
