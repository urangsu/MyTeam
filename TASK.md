...
- ArtifactStore absolute path 저장 구조 점검
- workspace-relative path normalize 적용
- 기존 absolute path artifact entry를 안전하게 읽고 relative path로 보정
- RecentArtifactIndex와 ArtifactStore consistency check 추가
- action_log.jsonl compaction / rotation 정책 추가
- 오래된 artifact cleanup dry-run policy 추가
- missing file / stale index / hash mismatch 상태 분리
- RuntimeDiagnostics에 artifact store health 표시
...
- Release build diagnostics 최소화 확인
- debug toggle hidden 확인
- PlanRunner default policy 확인
- artifact / file / scheduler core runtime smoke test 정리
- Mac App Store packaging checklist 작성
- 샌드박스 / 권한 / 파일 접근 정책 최종 확인
