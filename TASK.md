...
#### Round 42A-42H — Manual Runtime QA Recovery + First Launch Runtime Polish + Submission Gap Closure Pack

목표:

- Round 40R + 41A-41F 커밋의 main 반영 확인
- first launch / no-key / offline / connector-limited UX 실제 연결 상태 점검
- starter actions 4개 실제 UI 표시 및 route 연결 확인
- first result activation flow 실제 연결 확인
- Finder open / path copy 실제 UI 확인 준비
- fileImporter sandbox 실제 동작 확인 준비
- multi-room active task isolation 검증 준비
- wrong-room artifact reuse 차단 검증 준비
- recent artifact binding / hash mismatch / stale action 상태 점검
- Release / DEBUG visibility 실제 코드 경로 재점검
- StoreKit / entitlement 상태 문서화 보강
- Google Calendar OAuth live QA 준비 상태 정리
- 제출 전 manual QA backlog를 실행 가능한 체크리스트로 압축
- Debug / Release build app code warning 0 유지

### Next

#### Round 43A — Onboarding Visual Polish + Guided First Workflow Pack

목표:

- 첫 실행 화면의 시각적 polish
- 사용자가 1분 안에 첫 문서를 만들 수 있는 guided flow 정리
- starter action 버튼/카드 위치 최적화
- empty state copy 최종 다듬기
- onboarding runtime QA 결과 반영
