...
목표:
- Room-scoped recentArtifacts 전환 (전역 → 방별 격리, P0 신뢰 수정)
- 용어 정책 통일 (프로젝트→대화, 채팅방→워크룸, 스케줄 근무→예약 작업)
- TeamStatusView mini widget 정리 (핵심 5요소로 단순화)
- AgentChatView 에이전트 전환 switcher 제거
- 첫 empty state 단순화 (상태카드 1 + 주요 액션 3)
- Starter action 3 primary (파일 맡기기/문서 만들기/오늘 정리하기)
- TypingIndicatorView timer leak 수정
- RuntimeDiagnostics / ToolContractValidator / RouterBurnInSuite 보강
- docs: ProductIAPolicy / RoomScopedArtifactPolicy / WorkSurfaceSimplificationPlan / TerminologyPolicy
