...
- WorkflowOrchestrator route / execution 경계 일부 분리
- Daily Briefing workflow entrypoint를 WorkflowRunner로 이동
- Universal Document PlanRunner wrapper를 WorkflowRunner 중심으로 정리
- ConnectorCapabilityPolicy 추가
- ConnectorGuard / AssistantConnectorPolicy 정책 문구 단일화
- 큰 리팩터링 없이 P0 병목만 좁게 수정
...
- AgentWindowManager에 모인 room runtime state 경계 분리
- activeTasks / roomGoalContext / lastFileIntake / recentArtifacts 소유권 정리
- in-memory와 persistence 기준 명확화
