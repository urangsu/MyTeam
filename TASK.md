...
- Local Task Briefing route runtime QA
- 최근 파일 / 최근 artifact / 오늘 스케줄 / pending approval / pending delegation / 최근 실패 workflow 표시 확인
- 다음 액션이 실제 지원되는 기능만 말하도록 정리
- “방금 만든 문서 표로 바꿔줘” 지원 여부 확인
- 지원되면 recent artifact content resolver 추가, 아니면 next action에서 제거
- Gmail/Calendar 준비 중 문구 과장 제거
- 금지 route 회귀 확인
- Deferred Runtime QA Backlog 유지

### Next

#### Round 33B — Actionable Briefing + Local Scheduler Bridge

목표:

- BriefingActionSuggestion 모델 추가
- BriefingActionSuggestionProvider 추가
- Daily Briefing에 실행 가능한 action suggestions 연결
- DailyBriefingCardView에 action chips 표시
- action tap 시 prompt route 또는 system action 실행
- 최근 파일 / 최근 artifact / 오늘 스케줄 / pending approval / pending delegation action 지원
- unsupported action 숨김
- 자동 승인 / 외부 write 금지
- Gmail API / Calendar write / OAuth 구조 미구현 유지
- Deferred Runtime QA Backlog 유지

### Later

#### Round 33C — Actionable Briefing UX Polish + Local Scheduler Commands

목표:

- action chip UX polish
- 스케줄 업무 자연어 명령 강화
- 승인 대기 보기 / 스케줄 열기 연결 강화
- QA는 build + 핵심 회귀만 최소 수행
