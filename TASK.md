...
#### Round 34B — Local Scheduler Command + Approval Surface Pack

완료:

- LocalSchedulerCommand 모델 추가
- LocalSchedulerCommandDetector 추가
- LocalSchedulerCommandService 추가 (기본 구현)
- WorkflowOrchestrator 라우팅 연결
- RouteDecision / TurnProfile / RouteTrace / RouterBurnInSuite 업데이트
- LocalScheduler 명령 테스트 케이스 추가

미완료 (다음 라운드로):

- BriefingActionDispatcher system action 정리
- Daily Briefing action chip과 local scheduler command 연결
- isWorkflowRunning 의존도 축소
- RuntimeDiagnostics 보강
- LocalSchedulerCommandService 실제 업무 데이터 접근 구현
