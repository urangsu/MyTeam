...
목표:
 ...
 - currentWorkflowTask 전역 단일 제거
 - room별 activeTasks 관리
 - blocked capability early return
 - ResultVerifier error gate + 재생성 1회
 - RoomGoalContext 추가
 - activeWorkflowStep 기록
 - 최근 artifact 참조 감지 기반 추가
 - ClarificationPolicy를 sourceText/context 기반으로 보강
 - vague organize request guard 강화
 - File Intake / OAuth / Gmail / Calendar write는 미구현 유지
...
목표:
 ...
 - WorkPlan / WorkStep / StepKind 추가
 - VerificationLevel enum 추가
 - RecoveryAction 최소 모델 추가
 - PlanRunner skeleton 추가
 - UniversalDocumentPlanFactory 추가
 - FeatureFlags.planRunnerUniversalDocumentEnabled 추가
 - Universal Document를 PlanRunner로 점진 전환할 준비
...
목표:
 ...
 - RouteResolver / RouteDecision / GoalGate 추가
 - WorkflowRunner 분리
 - PlanEngine 분리 준비
 - ToolExecutionLayer skeleton
 - ConnectorGuard skeleton
 - ClarificationPolicy route 연결
...
목표:
 ...
 - AgentWorkOrder 확장
 - AgentRole 추가
 - inputKeys / outputKey / VerificationLevel
 - pipelineContext 적용
 - researcher → drafter → reviewer 기본 파이프라인
 - artifact에는 캐릭터 대사 금지
...
목표:
 ...
 - txt / md / csv 파일 입력
 - 위험 확장자 차단
 - PDF / DOCX / XLSX / PPTX 준비 중 안내
 - File Intake → Universal Document 연결 준비
