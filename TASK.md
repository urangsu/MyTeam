...
- WorkflowRunner 실행 경계 확대
- Universal Document PlanRunner wrapper를 WorkflowRunner 중심으로 정리
- WorkflowOrchestrator 직접 실행 책임 추가 축소
- RoomRuntimeStore actor/thread 경계 명확화
- diagnostics hardcoded capability flag 정리
- File Intake / Daily Briefing / Connector policy 회귀 없음
...
- PlanRunner와 AgentPipeline의 공통 step/context/verification contract 정리
- 중복 실행 엔진으로 커지는 위험 완화
- AgentPipeline 기본 route 연결은 아직 하지 않음
