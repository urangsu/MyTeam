...
- 다음 구현 대상은 Core Work Pack 내부 connector foundation으로 이동
...
- macOS Desktop OAuth 전제
- Web Server OAuth / CLI / gcloud 의존성 금지
- Google Calendar / Gmail 연결 기반
- Calendar read-only 1순위
- Gmail metadata 2순위
- Gmail body read-only는 추후 승인 필요
- send / create / update / delete 차단
- Settings에 비서 연결 섹션 추가
- 실제 OAuth/API 호출은 하지 않음
...
- 오늘 할 일
- 확인 필요한 항목
- connector 미연결 시 graceful empty state
- 위임모드와 연결 가능한 브리핑 구조

#### Round 25 — Google Calendar Read-only Integration
...
#### Round 27 — Universal Document Workflows
