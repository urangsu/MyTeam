...
- Daily Briefing 자연어 라우팅 강화
- Local / Offline Briefing Provider 추가
- Google Calendar 미연결 empty state 정리
- Gmail metadata 준비 중 상태 정리
- ConnectorGuard / AssistantConnectorPolicy 정렬
- mailSend / calendarWrite / automaticLogin / destructiveAction blocked 유지
- RuntimeDiagnostics에 briefing / connector 상태 표시
- Deferred Runtime QA Backlog 유지

### Next

QA 결과에 따라 둘 중 하나로 정한다.

문제가 남으면:

#### Round 29D — Runtime QA Fix Follow-up

목표:

- Round 30A에서 실패한 항목만 좁게 수정
- 새 기능 추가 금지

대부분 통과하면:

#### Round 30B — Daily Briefing Runtime UX

목표:

- 오늘 브리핑 카드 UX polish
- local briefing 결과 chat / artifact 선택 정책
- Google OAuth client ID 준비 시 Calendar live QA 재시도
- Gmail metadata는 OAuth 검증 이후
