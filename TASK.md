...
#### Round 34C-Repair (Partial) — Artifact Pipeline Real Integration Pack

**Completed (Steps 4 & 6):**

- ✅ 문서 유형별 verification fail-closed 실제 적용 (Step 4)
  - ResultVerifier에 document-type-specific 검증 메서드 6개 추가
  - ExecutionVerifier.verify(documentType:)로 타입별 검증 지원
  - PlanRunner에서 document type 전달 & 검증 로직 적용
  - Fail-closed 정책: error → no store + recovery 1회 / warning → store + note
- ✅ RuntimeDiagnostics 값을 실제 상태 기반으로 변경 (Step 6)
  - RoomGoalContext에 artifact/verification 상태 추적 필드 추가
  - RuntimeDiagnosticsService snapshot()에서 동적 값 계산 (hardcoding 제거)

**Remaining (Steps 5, 7, 8):**

- [ ] PlanExecutionResult / artifactCount cleanup (Step 5) — 현재 코드 검토 필요
- [ ] RouterBurnIn / ToolContractValidator 테스트 케이스 추가 (Step 7)
- [ ] 추가 문서화 및 QA (Step 8)
