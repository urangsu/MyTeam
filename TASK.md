...
- room별 최근 File Intake 결과 저장
- “방금 파일 / 이 파일 / 파일 내용” 참조 감지
- 파일 내용을 Universal Document sourceText로 연결
- 파일 기반 요약 / 보고서 / 표 / 체크리스트 / 회의록 / 액션아이템 생성
- artifact 완료 메시지에 파일명 표시
- 파일 읽기 직후 자동 생성은 하지 않음
- PDF / DOCX / XLSX / PPTX 파싱은 계속 미구현
...
- 파일 선택창에서도 planned/blocked 타입 안내 개선
- fileImporter allowed types 재검토
- drag/drop 안정성 QA
- 파일 결과 카드 polish
- 샌드박스 파일 접근 QA
