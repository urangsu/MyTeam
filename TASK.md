...
진행 중:
- 블로그/콘텐츠 글쓰기는 MyTeam의 메인 포지션이 아니라 문서/파일/표/정리 루프의 보조 기능으로 유지한다.
- 사용자-facing 문구는 "블로그 글쓰기 최적화"보다 "콘텐츠 초안 보조"를 우선한다.
- WorkroomHomeView 핵심 CTA는 문서 만들기 / 파일 맡기기 / 오늘 정리하기를 유지한다.
- `/blog-source`는 power user shortcut으로 유지하고, 공개 글 URL을 워크룸 참고 정보로 축적한다.
- 팀 워크룸과 개인 대화의 메시지 / artifact / LLM context room-scoped 격리는 P0로 유지한다.
...
- `LocalSkillExecutionResult.officeReviewResult` case 추가
- `executeIfPossible`: `.success(result)` → `.officeReviewResult` 반환
- 2차 assistOnly 스킬 도달 가능 경로 수정
- `WorkflowOrchestrator`: `.officeReviewResult` case 추가
- `OfficeReviewLiteExecutor.formatMarkdown(_:)` 추가
- `OfficeReviewResultCardView` macOS-safe 색상 + "위치:" → "휴리스틱 참고:"
- 미사용 바인딩 경고 수정
- 6개 ToolContractValidator + 6개 DiagnosticsService 필드 추가
- Preflight 20/20 ✅

#### Round 248A-OFFICE-LITE — 완료 (2026-05-21)

Office Review Lite Executor with Heuristic Extraction

**신규 Swift 파일 (2개):**
- `OfficeReviewLiteExecutor.swift` — 1차 heuristic (meetingActionItems, filenameOrganization, reportTonePolish) + 2차 assistOnly stubs
- `OfficeReviewResultCardView.swift` — limitations disclaimer with orange disclaimer box

**기존 파일 수정:**
- `LocalSkillExecutor.swift` — office review lite detection + 1차 execution routing
- `RuntimeDiagnosticsService.swift` — 9개 office review lite status fields
- `ToolContractValidator.swift` — 6개 office review lite validators
- `OfficeReviewInputPolicy.md` — Round 248A update notes

**신규 문서:**
- `docs/OfficeReviewLiteExecutorPolicy.md` — 1차/2차 differentiation, heuristic details
- `docs/OfficeReviewEvidenceHonestyPolicy.md` — evidence tracking unsupported, no file mutation, no fake parsing

**preflight:** preflight_round248a_office_lite.sh (12/12 ✅)
**pbxproj:** register_round247a_files.py 수정 (총 4개 파일, 247A 2개 + 248A 2개)

#### Round 247A-OBSERVE-RUNTIME — 완료 (2026-05-21)

Observation Runtime Wiring into User Flows

**신규 Swift 파일 (2개):**
- `ObservationInboxView.swift` — pending observations (roomID=nil) with attach/ignore buttons
- `ObservationPresentationPolicy.swift` — centralized observation-related messages

**기존 파일 수정:**
- `TeamStatusView.swift` — ObservationInboxView wired to selectedTeamWorkroomID
- `AgentChatView.swift` — ObservationInboxView wired to agentRoomID
- `WorkflowOrchestrator.swift` — handleExplicitContextRoute() for clipboard/Finder/screen
- `AgentWindowManager.swift` — pendingObservationsForCurrentSurface(), attachObservation(), analyzeObservation(), ignoreObservation()
- `RuntimeDiagnosticsService.swift` — 10개 observation runtime fields
- `ToolContractValidator.swift` — 8개 observation validators

**신규 문서:**
- `docs/ObservationRuntimeUXPolicy.md` — room-scoped UI wiring, ObservationInboxView policy
- `docs/ClipboardExplicitReadPolicy.md` — explicit-only routing, credential blocking
- `docs/FinderSelectionFallbackPolicy.md` — Cloud fallback, Mac local roadmap

**preflight:** preflight_round247a_observe_runtime.sh (13/13 ✅)

#### Round 243A-OBSERVE — 완료 (2026-05-20)

Local Observation Foundation Pack

**신규 Swift 파일 (9개):**
- `ObservationModels.swift` — LocalObservation, ObservationSource, ObservationContentKind
- `ObservationPermissionPolicy.swift` — hard blocks (상시감시/캡처/자동업로드 금지)
- `LocalObservationService.swift` — room-scoped, pending attach
- `DownloadsFolderWatcher.swift` — 기본 OFF, FSEvents skeleton
- `ClipboardContextReader.swift` — 명시 요청만, credential 차단
- `FinderSelectionReader.swift` — skeleton
- `ScreenObservationPolicy.swift` — planned, 상시 캡처 하드블록
- `FileIntakeEventCardView.swift` — 관찰 카드 UI
- `OfficeReviewInputPolicy.swift` — 9개 사무 검토 skill + 결과 카드 구조

**pbxproj:** 14개 파일 Python plistlib으로 직접 등록 (244A 5 + 243A 9)
**preflight:** 14/14 ✅
**xcodebuild:** Cloud 환경 미실행 — **Mac 로컬에서 빌드 확인 필요**

Mac 확인 후 할 일:
- `xcodebuild Debug/Release` 통과 확인
- `scripts/mac_register_round243a_files.rb` 불필요 시 삭제 가능

#### Round 241A-CORE — 완료 (2026-05-19)

팀 워크룸 / 개인 대화 하드 분리 완료.
- `selectedTeamWorkroomID` + `activePersonalAgentID` 추가
- TeamStatusView 전체 `selectedTeamWorkroomID` 기준 전환
- 개인 대화 사이드바 message preview 제거
- Preflight 12/12 통과, Debug ✅ / Release ✅

#### Round 244A-MEMORY — 완료 (2026-05-20)

Memory Scope + User Preference + Procedural Learning Foundation

**신규 Swift 파일 (5개):**
- `MemoryModels.swift`: MemoryScope / MemorySensitivityClass / MemoryItem / MemoryCandidate / MemoryContext
- `MemoryScopePolicy.swift`: heuristic classify() + detectDomain()
- `MemoryStore.swift`: credentialLike 하드 블록 + roomID 강제 + 승인 대기 경로
- `MemoryConsolidator.swift`: 대화에서 candidate 추출 (원문 금지, 120자 잘라냄)
- `MemoryRetriever.swift`: priority retrieval + cross-room 격리

**기존 파일 수정:**
- `RouterBurnInCase.swift`: `expectedMemoryScope: String?` 필드 추가
- `RouterBurnInSuite.swift`: 6개 memory burn-in case 추가
- `RuntimeDiagnosticsService.swift`: 11개 memory 상태 필드
- `ToolContractValidator.swift`: 6개 memory scope validator
...
#### Round 241C-SURFACE — 완료 (2026-05-20)

Team Composer Routing + Unread Badge + Overlay/Chrome Repair Pack
- **TeamTableView.sendTeamInput**: `currentRoomID` → `selectedTeamWorkroomID` (팀 메시지 wrong room 버그 수정)
- **Unread badge**: `lastReadAtByRoomID` + `markRoomRead()` + `unreadCount()` 추가, 내 메시지/system 제외
- **Agent menu clipping**: `AgentMenuPopupView` 커스텀 overlay 제거 → SwiftUI `.contextMenu` 교체
- **Footer chrome**: 기존 `safeAreaInset` 구조 확인, 정책 문서 신규 작성
- RuntimeDiagnosticsService: 241C 8개 필드
- ToolContractValidator: 241C 5개 validator
...
- `selectedPersonalConversationIDByAgentID: [String: UUID]` 추가 (에이전트별 방 ID 매핑)
- `openPersonalConversation(for:)` 공식 API 추가, `openPersonalChat` → wrapper
- `personalConversation(for:)` + `currentPersonalConversation()` 헬퍼 추가
- **GoalGate**: `.blocked` → `.directChat` pivot — AI가 초안/도움말 계속 제공
- **WorkflowOrchestrator**: early-return을 `kind == .blocked`일 때만 적용
- **BYOKProviderCenterView**: `Button("") {} .disabled(true)` 제거, help tooltip 추가
- RuntimeDiagnosticsService: 241B 4개 필드 추가
- ToolContractValidator: 241B 4개 validator 추가
- scripts/preflight_round241b.sh 신규 (12/12 통과)
- docs/SupertonicAssessment.md 신규 (PoC 대기)
- docs/RoomIdentitySeparationPolicy.md: selectedPersonalConversationIDByAgentID 섹션 추가
- Preflight 12/12 통과, Debug ✅ / Release ✅

#### Round 238 — 다음 우선순위 결정 필요

Round 237 Local Runtime QA 완료. 다음 라운드 결정 대기.

참고: 미테스트 항목 (P2):
- Light mode 가독성 육안 확인
- FloatingPanel 축소/확장 크래시 10회 테스트
- /blog-source + /blog-profile 실제 응답 확인 (API 키 필요)

### Recently Completed

#### Round 237 — Local Runtime QA

완료 (2026-05-18): 앱 실행 후 수동 QA

- Debug 앱 실행 확인 (PID 55102, Antigravity/com.google.antigravity)
- 팀 협업 패널: 4개 에이전트(레오/루나/모코/핀) 정상 표시
- 온보딩 카드 닫기 정상
- 에이전트 초상화 클릭 → 관리 컨텍스트 메뉴 (추가/교체/팀장) 확인
- 에이전트 교체 카탈로그 모달 표시 확인
- 메시지 전송 → Gemini API 호출 확인 (429 rate limit 우아한 처리 확인)
- 설정 > Google Calendar: "읽기 준비 중, 쓰기 자동 실행 안함" 안내 ✅
- 설정 > Keychain 저장 안내 ✅
- 기술 UX 금지 문구 미노출 확인
- 커넥터 쓰기 API 직접 호출 없음 (ConnectorReadinessPlan 준수)
- Preflight Round 236 12/12 통과
- Preflight Round 237 10/10 통과
- docs/qa/LocalRuntimeQA_Round237.md 작성
- scripts/preflight_local_runtime_round237.sh 작성

발견 이슈:
- P2: 설정 창 X 버튼 클릭 시 macOS Window 메뉴 열림 (좌표 충돌)
- INFO: 개발 환경 레이아웃 제한으로 채팅 로그 영역 육안 확인 어려움
- INFO: Gemini 429 rate limit으로 AI 응답 테스트 불가
...
완료 (2026-05-17): Build-ready, Manual QA pending

- ChatModels.swift: RoomProfile / BlogStyleProfile / BlogSEOProfile / RoomMode 구조 추가
- AgentWindowManager: renameRoom(id:newName:), openPersonalChat(for:), inferredRoomProfile(for:) 구현
- ConversationMemory: /blog-source, /blog-profile, /purpose, /purpose reset 명령 처리
- TeamStatusView + AgentChatView: 블로그 profile이 방 맥락에 반영되는 구조
- 블로그 "전용 방" 강제 생성 제거 → room purpose inference (제안 수준)
- RouterBurnInSuite 9개 케이스 추가 (room rename/purpose/blog/agent tap/blocked write)
- ToolContractValidator 7개 validator 추가 (rename/scoped/purpose/blog/connector/polite)
- RuntimeDiagnosticsService Round 236 필드 9개 + room236 summary 라인
- scripts/preflight_room_ui_round236.sh 12단계 (전체 통과)
- docs/ProductImplementationInventory.md (신규)
- docs/connectors/ConnectorReadinessPlan.md (신규)
- Debug + Release BUILD SUCCEEDED 0 warnings

#### Round 235 — UI Readability P0 Fixes + Agent Chat Switching

완료 (2026-05-17): Build-ready, Manual QA pending

- Color+Hex.swift에 MT Readability Tokens 추가 (mtTextPrimary/Secondary/Tertiary, mtCardBackground, mtInputBackground, mtCardBorder)
- TeamStatusView 채팅 버블 배경을 mtCardBackground로 교체 (low opacity glass 제거)
- TeamStatusView 입력 필드를 mtInputBackground로 교체, foreground color 명시
- AgentChatView inputBgColor를 mtInputBackground로 통일, subTextColor를 mtTextSecondary로 상향
- BeginnerTaskCardView bgColor를 mtCardBackground로 교체, 내부 텍스트 opacity 상향
- WorkroomHomeView 카드 배경 3개소 mtCardBackground로 교체, nextActions 텍스트 명시적 색상 지정
- StatusAgentRow 배경 mtCardBackground, 텍스트 mtTextPrimary/Secondary/Tertiary 적용
- TeamStatusView 에이전트 리스트 탭 → openPersonalChat(for:) 연결 (currentRoomID 전환)
- TeamTableView 에이전트 시트 탭 → openPersonalChat(for:) 추가 (chat room 전환)
- RuntimeDiagnosticsService Round 235 필드 5개 + ui235 summary 라인 추가
- scripts/preflight_ui_readability_round235.sh 신규 (12단계)
- Debug + Release BUILD SUCCEEDED 0 warnings
