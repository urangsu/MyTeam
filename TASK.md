...
#### Round 61A-75H — Product Surface Completion + Character System + Launch Asset Readiness Mega Pack

Round 56A-60H 완료 후 앱의 제품 표면, 캐릭터 시스템, 출시 자산을 모두 닫는 라운드.
실제 UI 연결, 상태 모델 통일, 문서화, 그리고 빌드 검증까지 마친다.

목표:

- StarterActionStripView를 TeamStatusView / DailyBriefingCardView에 실제 연결
- FirstLaunchBannerView / LocalOnlyModeCardView 표시 조건 정리
- "회의록 양식 → artifact 생성 → next action" 제품 흐름 연결
- First result activation flow를 ArtifactCardView에 연결
- ArtifactCardView 상태/액션 표면 정리
- Workspace home / recent artifact section 최소 정리
- SettingsView를 일반 사용자 기준으로 간결화
- Connector Center 상태 모델을 available/readOnly/planned/requiresApproval/blocked/unavailable로 통일
- approval / blocked / unavailable / planned 메시지 통일
- RuntimeDiagnosticsService에 product surface / character / release 상태 추가
- ToolContractValidator에 connector/write/placeholder/DLC/privacy copy 정책 검사 추가
- 치코 중심 기본 캐릭터 경험을 화면에 반영
- 스프라이트 없는 캐릭터 / DLC Release 노출 차단 정책 고정
- 캐릭터 에셋 제작 명세서 작성
- 앱스토어 스크린샷 준비 문서 작성
- App Store metadata / privacy / landing copy와 앱 내 copy 정합성 확인
- ToolExecutor Swift 6 warning 원인 확인 및 가능하면 제거
- deployment target / copyright / permission copy 재점검
- QA 없이 Debug / Release build warning 0 유지

진행 체크리스트:

- [ ] 1. TASK.md 대정리 (Now/Next 전환)
- [ ] 2. 시작 상태 확인 (git status, branch, main 동기화)
- [ ] 3. pbxproj / target 등록 재검증
- [ ] 4. StarterActionStripView 실제 UI 연결
- [ ] 5. StarterActionDispatcher 안정화
- [ ] 6. FirstLaunchBannerView 표시 조건 정리
- [ ] 7. LocalOnlyModeCardView 실제 연결
- [ ] 8. First result activation flow 구현
- [ ] 9. ArtifactCardView product surface 정리
- [ ] 10. Workspace home / recent artifact section 최소 정리
- [ ] 11. DailyBriefingCardView empty state 통합
- [ ] 12. SettingsView 일반 사용자 기준 간결화
- [ ] 13. Connector Center 상태 모델 통일
- [ ] 14. Approval / Blocked / Unavailable 메시지 통일
- [ ] 15. 치코 중심 기본 캐릭터 경험 강화
- [ ] 16. 캐릭터/DLC 노출 정책 구현
- [ ] 17. StoreKit / Paywall surface 정리
- [ ] 18. TTS / Audio fallback 상태 정리
- [ ] 19. Character asset readiness 문서 작성
- [ ] 20. App Store / Landing / Privacy 문구 정합성 확인
- [ ] 21. Deployment target / copyright / permission copy 점검
- [ ] 22. RuntimeDiagnosticsService 보강
- [ ] 23. ToolContractValidator 보강
- [ ] 24. RouterBurnInSuite 보강
- [ ] 25. ReleaseWarningAudit / RuntimeQAPlaybook 문서 정리
- [ ] 26. DEVLOG / SystemBottleneckAudit 반영 및 최종 커밋

### Next

#### Round 76A — Character Asset Production Pack

목표:

- 치코 외 최소 2~3개 캐릭터 스프라이트 제작 반영
- 캐릭터별 업무 역할 차별화
- character team screen visual polish
- 앱스토어 스크린샷 후보 화면 정리
