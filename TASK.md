...
- PlanRunner와 AgentPipeline 공통 execution contract 추가
- ExecutionStepContract 추가
- ExecutionContextBag 추가
- ExecutionVerifier 추가
- WorkStep / AgentWorkOrder 변환 helper 추가
- legacy fallback outcome을 Void가 아닌 명시 result로 변경
- artifactCount 오기록 방지
- verification/safety failure fallback 금지 유지
- AgentPipeline 기본 route 연결 없음
...
- Daily Briefing에 앱 내부 할 일 / 최근 workflow / 스케줄 업무를 더 자연스럽게 연결
- 외부 API 없이 사용자 가치 강화
