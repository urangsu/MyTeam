...
- LocalTaskBriefingModels 추가
- LocalTaskBriefingProvider 추가
- Daily Briefing에 최근 파일 / 최근 artifact / 오늘 스케줄 / pending approval / pending delegation / 최근 실패 workflow 연결
- “오늘 할 일 / 지금 이어서 할 일 / 오늘 작업 정리” 자연어 라우팅 보강
- 다음 액션 섹션을 실제 이어서 할 수 있는 문장으로 개선
- Gmail API / Calendar write / OAuth 구조는 미구현 유지
- Deferred Runtime QA Backlog 유지

### Next

#### Round 32B — Local Task Briefing Runtime QA + Polish

목표:

- Local Task Briefing route runtime QA
- 스케줄 업무 / 최근 파일 / 최근 artifact / pending approval 표시 확인
- 금지 route 회귀 확인
- UI 문구 polish
