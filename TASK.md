...
#### Round 29C — Runtime QA, Fix Pack, and Release Stability Sweep

목표:

- Round 29B에서 남은 런타임 미확인 항목 재검증
- File Intake 실제 UI / sandbox / drag-drop 검증
- File Intake → Universal Document artifact 생성 검증
- Artifact Finder 열기 / 경로 복사 검증
- multi-room active task isolation 검증
- PlanRunner flag false / true 경로 검증
- blocked capability early return 실제 앱 경로 검증
- 필요한 작은 수정만 반영
- DEBUG QA helper 추가 가능
- 새 기능 추가 금지
