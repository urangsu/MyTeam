...
- BriefingActionDispatcher 추가
- Daily Briefing actionSuggestions 연결
- DailyBriefingCardView action chips 표시
- action tap 시 prompt route 또는 system action 실행
- 최근 파일 / 최근 artifact / 오늘 스케줄 / pending approval / pending delegation action 지원
- Local Scheduler Bridge 연결
- Recent Artifact Reuse action과 실제 route 일치
...
#### Round 34A — Local Scheduler Command Pack

목표:

- 스케줄 업무 자연어 명령 강화
- 승인 대기 보기 UX 정리
- 스케줄 생성/수정은 local-only 범위에서만
- 외부 calendar write 금지
- Daily Briefing action과 scheduler command 연결 강화
