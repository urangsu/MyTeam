...
- keyFacts / user memory 저장 정책 보강
- automationTasks prompt 저장 redaction
- UserDefaults 민감정보 저장 차단
- Release / DEBUG path hardening
- AIService Release model pinning
- Diagnostics minimization
- Deferred Runtime QA Backlog 재분류
- RouterBurnInSuite / ToolContractValidator 보강

### Next

#### Round 38A — ArtifactStore Relative Path + Compaction Pack

목표:

- ArtifactStore absolute path 저장 구조 점검
- relative path migration 설계
- append-only artifact log 검토
- recent index compaction
- 오래된 artifact cleanup 정책
