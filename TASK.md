...
#### Round 136A — Mac Local Merge + Build Repair Pack

목표:

- Cloud branch merge to main (git merge --no-ff)
- pbxproj target audit 실행 (python3 scripts/pbxproj_target_audit.py)
- 누락 Swift file target 등록 (ruby scripts/mac_register_round116_files.rb)
- Debug xcodebuild 실행 및 검증
- Release xcodebuild 실행 및 검증
- ToolExecutor warning 확인/수정
- Character/Policy/Validator compile 확인

환경:
- Mac local (xcodebuild 필수)
- Build succeeded가 완료 조건
- Merge conflict 해결 포함

### Next

#### Round 140A — Manual Runtime QA Pack

목표:

- First launch 실제 확인
- Starter action 실제 클릭 확인 (5개 allowed cases)
- First result activation 확인 (valid/invalid artifact states)
- Character 가시성 확인 (Chiko visible, others hidden)
- Connector 노출 확인 (calendar/mail/delete hidden)
- StoreKit disabled button 표시 확인

환경:
- Mac local (runtime QA)
- 모든 policy 동작 검증
