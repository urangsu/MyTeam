...
#### Round 136A-UXFIX — Product Surface P0 Repair

목표:
- 팀 이름 명패 설정 compact palette + border on/off로 정리
- DART 공시 기능 public disclosure read로 활성화 (defaultEnabled: true)
- 기본 캐릭터 치코 isPremium: false, 역할/상태 현행 카탈로그 기준 반영
- API key 안내를 Settings 중심으로 이동, Team collaboration surface에서 제거
- RuntimeDiagnostics / ToolContractValidator / RouterBurnInSuite 보강
- Debug/Release build 확인
...
- 회의록 양식 artifact 생성 확인
- First result activation 확인
- Finder open 확인
- path copy 확인
- File intake sandbox 확인
- Connector blocked/read-only 상태 확인
- StoreKit sandbox purchase QA 준비 상태 확인
- Google OAuth live QA는 client ID 준비 전까지 pending 유지
