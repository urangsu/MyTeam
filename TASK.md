...
#### Round 34D-34F — Artifact UX + Recent Reuse Persistence + Release Path Pack

목표:

- Xcode user state 파일 git 추적 제거 ✅
- ArtifactCardView UX polish ✅
- WorkspaceFileActions 신규 추가 (Finder, path copy) ✅
- Recent artifact reuse 실패 메시지 4가지 명확화 ✅
- RecentArtifactIndexPersistence v1 metadata-only persistence ✅
- RuntimeDiagnostics artifact UX / persistence / release path 상태 추가 ✅
- RouterBurnInSuite recent artifact reuse 케이스 6개 추가 ✅
- Release / DEBUG PlanRunner path distinction 명확화
- 외부 write / Gmail API / Calendar write / OAuth 구조 미구현 유지

### Next

#### Round 35A — File Intake Planned Types Foundation

목표:

- PDF/DOCX/XLSX/PPTX planned type UX 정리
- 실제 파서는 아직 넣지 않음
- 사용자가 "준비 중 / 지원됨 / 차단됨"을 명확히 이해하도록 개선
