...
#### Round 43R-47H-FINAL — Product Surface Integration + Safety Copy + Release-Ready Implementation Pack

이 라운드는 QA 라운드가 아니다. 구현 통합 마감이다.

목표: 모든 UI 컴포넌트를 실제 제품 화면에 연결하고, 릴리스 가능한 상태로 만든다.

진행 체크리스트:

- [x] 0. 현재 상태 확인 (merge commit daa2b52) ✅
- [x] 1. TASK.md 대정리 (현재 라운드: Round 43R-47H-FINAL) ✅
- [x] 2. Swift 파일 Xcode target 점검 및 pbxproj 중복 제거 ✅ (9개 누락 파일 확인: FirstLaunchState, FirstLaunchStateProvider, FirstLaunchBannerView, LocalOnlyModeCardView, RuntimeCapabilityMode, StarterAction, StarterActionDispatcher, StarterActionStripView, Color+Hex)
- [ ] **3. ⚠️ BLOCKER: pbxproj에 누락된 9개 파일 추가 필요** (Xcode UI에서 수동으로 "Add Files to MyTeam" 수행 필요) → 이후 FirstLaunchStateProvider 실제 연결
- [ ] 4. FirstLaunchBannerView 화면 통합 (TeamStatusView, DailyBriefingCardView, SettingsView)
- [ ] 5. LocalOnlyModeCardView 화면 통합
- [ ] 6. StarterActionProvider 완성 (4개 action max)
- [ ] 7. StarterActionStripView 화면 통합 + StarterActionDispatcher 라우팅
- [ ] 8. StarterActionDispatcher 라우팅 안전성 검증
- [ ] 9. First Result Activation Flow 완성 (summary, table, checklist, finder)
- [ ] 10. ArtifactCardView 제품 표면 (파일 상태, 액션, 전체 경로 없음)
- [ ] 11. WorkspaceHomeView / ArtifactLibrarySectionView 최소 구현
- [ ] 12. DailyBriefingCardView 빈 상태 통합
- [ ] 13. SettingsView 일반 사용자 기준 간결화 (OAuth client ID 숨김, 릴리스는 debug 숨김)
- [ ] 14. Connector Center 상태 모델 통일 (6가지: available, readOnly, planned, requiresApproval, blocked, unavailable)
- [ ] 15. Approval / Blocked / Unavailable 메시지 표준화
- [ ] 16. StoreKit / Paywall 표면 문구 (로직 변경 없음)
- [ ] 17. TTS/Audio fallback 상태 정리
- [ ] 18. App Store Metadata / Privacy Nutrition 초안 강화
- [ ] 19. RuntimeDiagnostics release/debug 상태 정리
- [ ] 20. RouterBurnInSuite case 추가 (12개 신규: first launch, file intake, reuse, blocked actions, TTS 등)
- [ ] 21. ToolContractValidator 검증 추가
- [ ] 22. RuntimeQAPlaybook / WorkflowQABurnIn 문서화 (다음 수동 QA 범위만, 실행 없음)
- [ ] 23. DEVLOG / SystemBottleneckAudit 최종 반영
- [ ] 24. 금지사항 강제 (QA 실행 X, 외부 write X, Gmail/Calendar write X, OAuth 변경 X 등)
- [ ] 25. Build 검증 (Debug, Release 모두 성공, warning 0)
...
- Finder open / path copy 실제 UI 확인
- fileImporter sandbox 실제 확인
- multi-room active task isolation 확인
- wrong-room artifact reuse 실제 확인
- first-launch onboarding runtime 확인
- Google Calendar live OAuth 준비 상태 확인
- StoreKit production purchase QA 준비
