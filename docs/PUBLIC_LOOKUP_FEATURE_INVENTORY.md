...
- dart.company diagnostic: NON-BLOCKING DIAGNOSTIC, current live curl timed out after 12s from the client check.
- dart.recent diagnostic: CONDITIONAL PASS, HTTP 502, `provider_system_error`, provider `dart`, status `522`, stage `dart_fetch`, classification `provider_reachability_failure`, mergeGate `conditional-pass`
- dart.diagnose: NON-BLOCKING DIAGNOSTIC, current live curl timed out after 12s from the client check.
...
- blocker: none for the proxy-to-main merge gate. DART user-facing execution is app direct BYOK because Cloudflare Worker provider requests still fail or time out when reaching OpenDART. Worker DART routes are diagnostics only.
