...
| ART-006 | Index entry exists but file missing | Create or simulate recent artifact index entry with missing file | Detail view shows missing artifact recovery state, not fake content | Blank success, Finder-only failure, crash | Automated store fixture detects post-cache file changes; UI not run | BLOCKED | Commit `4e7f4ff`; `test_artifactStoreRefreshesCachedHealthAfterFileChanges` | Reason: runtime detail-card recovery presentation is not manually proven. Next: delete a registered fixture file and inspect the card in Debug and Release. |
| ART-007 | File exists but index missing | Place artifact file without recent index entry | App does not show a broken recent item; recovery/import path is explicit if offered | Phantom recent item or crash | Automated compaction preserves recovery metadata when the artifact index is missing/corrupt; UI not run | BLOCKED | Commit `4e7f4ff`; missing/corrupt index XCTest fixtures | Reason: explicit user-facing rebuild/import action is unproven. Next: launch with a controlled missing index and confirm whether recovery UX is required. |
| ART-008 | Content hash mismatch | Modify artifact file after index registration | Detail view flags mismatch or revalidates; does not claim verified unchanged content | Stale hash ignored while showing verified-looking artifact | Automated fixture returns `hashMismatch`; UI not run | BLOCKED | Commit `4e7f4ff`; `test_artifactStoreRefreshesCachedHealthAfterFileChanges` | Reason: runtime detail-card wording and recovery action are not manually proven. Next: mutate a registered file and inspect the card in Debug and Release. |
| ART-009 | Concurrent artifact saves | Complete two natural work artifacts concurrently | Both artifacts keep distinct IDs, room IDs, hashes, and recent entries | Last writer wins, duplicate IDs, wrong room assignment | Artifact writers now resolve workflow ownership by requested room, reject index-registration failures, and avoid duplicate XLSX/PPTX registration; concurrent UI run not performed | BLOCKED | Commits `7522b8f`, `c28ce6b`; `test_artifactWorkflowOwnershipUsesTheRequestedRoom`; architecture validator | Reason: code ownership and persistence truth are hardened, but two real simultaneous saves and recent-card rendering still require runtime QA. |
